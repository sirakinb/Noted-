...
export function RecordingModal({ onClose }: RecordingModalProps) {
  ...
  const handleSaveRecording = async () => {
    ...
    try {
      // Upload to S3 using our custom API
      ...
      console.log("Transcribing with:", { audioUrl: url, language, durationSeconds: duration });
      
      const { id } = await transcribeMutation.mutateAsync({
        audioUrl: url,
        language,
        durationSeconds: Math.max(duration, 1), // Ensure minimum 1 second
      });
